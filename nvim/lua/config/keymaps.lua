...
vim.keymap.set ({ "n" }, "<C-s><C-a>", ":%s/<C-r><C-w>//g<Left><Left>", {
    noremap = true,
    silent = false,
    desc = "Replace word from under the cursor without confirmation",
})

vim.keymap.set ({ "n" }, "<C-s><C-s>", ":%s/<C-r><C-w>//gc<Left><Left><Left>", {
    noremap = true,
    silent = false,
...
vim.keymap.set ({ "n" }, "<C-h><C-h>", "<cmd>lua vim.lsp.inlay_hint.enable(false)<cr>", { noremap = true, silent = true, desc = "Disable inlay hints" })
...
vim.keymap.set ({ "n" }, "<F8>", "<esc> :w <cr> :Dispatch comp.sh $(echo %)<cr>", { noremap = true, silent = true, desc = "Compile and run" })
