return {
    {
        ...
        config = function ()
            ...
            vim.keymap.set (
                ...
                { noremap = true, silent = true, desc = "[S]earch [/] in Open Files" }
            )

            vim.keymap.set (
                { "n" },
                "<leader>cf",
                function () require ("telescope.builtin").fd ({ cwd = vim.fn.stdpath ("config") }) end,
                { noremap = true, silent = true, desc = "[S]earch [N]eovim files" }
            )
        end,
    },
}
